...
if($_SERVER['REQUEST_METHOD'] == "POST"){
  ...
  mysqli_query($conn, "INSERT INTO user (nama, nik, nomor_hp, rumah_id, email, password) VALUES ('$nama', '$nik', '$nomor_hp', '$rumah_id', '$email', '$password')");
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="form-group has-feedback">
        <input type="text" class="form-control" placeholder="NIK" required="required" name="nik" minlength="16" maxlength="16">
      </div>
      <div class="form-group has-feedback">
        <input type="text" class="form-control" placeholder="Nomor HP" required="required" name="nomor_hp" minlength="11" maxlength="12">
      </div>
      <div class="form-group has-feedback">
        <select name="rumah_id" class="form-control" required="required">
          ...
          <?php while($rowRumah = mysqli_fetch_assoc($resRumah)){
            $idrumah = $rowRumah['id'];
            $cek = mysqli_query($conn, "SELECT * FROM user INNER JOIN rumah ON user.rumah_id=rumah.id WHERE rumah_id='$idrumah'");
            $jumlah = mysqli_num_rows($cek);
          ?>
            <option value="<?php echo $rowRumah['id']; ?>" <?php if($jumlah>0){echo "disabled";}?>><?php echo $rowRumah['alamat']; ?></option>
          <?php }?>
        </select>
      </div>
      <div class="form-group has-feedback">
